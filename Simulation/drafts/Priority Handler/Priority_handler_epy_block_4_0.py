"""
Embedded Python Block: Receiver (Reads Length + Sends Priority 0 ACKs)
"""
import numpy as np
...
class blk(gr.basic_block):
    ...
    def handle_msg(self, msg):
        ...
        try:
            ...
            # === LEN FIX ===
            # Only read 'data_len' bytes. Ignore padding.
            end_idx = 7 + data_len
        ...
        if seq == expected:
            ...
            # Duplicate packet received, re-send ACK for what we actually need
    ...
    def send_ack(self, target_node, next_required_seq):
        ...
        # === PRIORITY 0 ===
        # ACKs must jump the queue in the Arbiter
        meta = pmt.make_dict()
        meta = pmt.dict_add(meta, pmt.intern("priority"), pmt.from_long(0))
        
        out_pmt = pmt.cons(meta, pmt.init_u8vector(len(header), list(header)))
